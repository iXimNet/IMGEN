...
use block/leaf offload with CPU VAE encode/decode; MPS and CPU stay resident.
Imports are lazy so the studio and BF16/INT8 paths do not require SDNQ.
"""
...
def apply_offload(pipe, spec: dict, device):
    ...
    if mode == "resident":
        ...
        # Untiled VAE activations compete with the denoiser cache on 8GB GPUs.
        # Preserve upstream pixels by moving both encode and decode to CPU.
        pipe.vae.to("cpu")
